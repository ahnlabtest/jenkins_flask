...
//        DEPLOY_NUMBER = "${BUILD_NUMBER}"   
    ...
    stages {
        ...
        stage("DEPLOY") {
            ...
            steps {
                script {
                    if("${params.OLD_BUILD_NO}"!=''){
                        env.DEPLOY_NO = "${params.OLD_BUILD_NO}"
                    } else {
                        env.DEPLOY_NO = "${BUILD_NUMBER}"
                    }
                }
                echo "# DEPLOY stage: env.DEPLOY_NO=${env.DEPLOY_NO} "
                withEnv(["DEPLOY_NO=${env.DEPLOY_NO}"]) {
                    sh(script: 'echo ${DEPLOY_NO}')
